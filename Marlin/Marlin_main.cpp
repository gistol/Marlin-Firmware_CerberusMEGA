...
void deploy_z_probe() {
  ...
  destination[X_AXIS] = 15;  //15 or 25
  destination[Y_AXIS] = 205; //198 or 207
  destination[Z_AXIS] = 40;
  prepare_move_raw();

  feedrate = homing_feedrate[X_AXIS]/2;
  destination[X_AXIS] = -15; //-15 or 0
  prepare_move_raw();
  st_synchronize();
}
...
void retract_z_probe() {
  ...
  destination[X_AXIS] = -117;
  destination[Y_AXIS] = -137.0;
...
float z_probe() {
  ...
  feedrate = homing_feedrate[Z_AXIS]/7;
  destination[Z_AXIS] = -20;
  prepare_move_raw();
  st_synchronize();
  endstops_hit_on_purpose();
